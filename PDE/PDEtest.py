...
'''
x_in = tf.Variable(tf.random_uniform(shape=[1000, 1], minval=0.0, maxval=1.0), dtype=tf.float32, trainable=False)
y_in = tf.Variable(tf.random_uniform(shape=[1000, 1], minval=0.0, maxval=1.0), dtype=tf.float32, trainable=False)
...
net_out = tf.matmul(tf.nn.relu(a), w1)

drelu = tf.clip_by_value(a, 0, a)
pd_x = w0x * (tf.transpose(w1) * drelu)
pd_y = w0y * (tf.transpose(w1) * drelu)
drelu2 = tf.clip_by_value(a, 0, a)
