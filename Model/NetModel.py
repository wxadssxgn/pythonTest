...
class MLP(nn.Module):
    def __init__(self, in_dim, n_hidden_1, n_hidden_2, n_hidden_3, n_hidden_4, n_hidden_5, n_hidden_6, n_hidden_7, out_dim):
        ...
        self.layer3 = nn.Sequential(nn.Linear(n_hidden_2, n_hidden_3), torch.nn.Dropout(0.0), nn.ReLU(inplace=True))
        self.layer4 = nn.Sequential(nn.Linear(n_hidden_3, n_hidden_4), torch.nn.Dropout(0.0), nn.ReLU(inplace=True))
        self.layer5 = nn.Sequential(nn.Linear(n_hidden_4, n_hidden_5), torch.nn.Dropout(0.0), nn.ReLU(inplace=True))
        self.layer6 = nn.Sequential(nn.Linear(n_hidden_5, n_hidden_6), torch.nn.Dropout(0.0), nn.ReLU(inplace=True))
        self.layer7 = nn.Sequential(nn.Linear(n_hidden_6, n_hidden_7), torch.nn.Dropout(0.0), nn.ReLU(inplace=True))
